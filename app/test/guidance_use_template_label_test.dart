...
void main() {
  test(
    'useATemplate reads as one clean phrase in every supported locale',
    () async {
      for (final locale in AppLocalizations.supportedLocales) {
        ...
        expect(
          label.trim(),
          isNotEmpty,
          reason: 'empty for ${locale.toString()}',
        );
        ...
        for (var i = 1; i < words.length; i++) {
          expect(
            ...
            reason:
                'duplicated word "${words[i]}" in ${locale.toString()}: '
                '"$label"',
          );
        }
  ...
  test('Simplified Chinese useATemplate is 使用模板 with no duplication', () async {
